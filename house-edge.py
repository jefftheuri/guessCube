...
def house_win(user_choice, results):
    # Implement house edge
    win_chance = 0.4  # 40% chance of winning
    if random.random() > win_chance:
        return 0  # House wins
    else:
        return results[user_choice - 1][1]  # User wins the prize
...
def process_sms(phone_number, user_input):
    ...
    try:
        ...
        if 1 <= number <= 6:
            ...
            # Apply house edge
            won_prize = house_win(number, results)
            if won_prize > 0:
                ...
                response += f"\nSorry, you didn't win this time. Better luck next round!"
